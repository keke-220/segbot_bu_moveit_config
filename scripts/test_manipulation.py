...
class arm_client(object):
    def __init__(self):
        self.init_joints = [0.20, -2.19, 2.04, -1.42, -1.57, 0.19]
        self.up_joints = [-0.04, -2.10, 1.22, -0.68, -1.47, -0.07]
        ...
        self.gripper_command = self.genCommand('a', self.gripper_command)
    ...
    def close_gripper(self):
        ...
        self.gripper_command = self.genCommand('c', self.gripper_command)
        self.gripper_pub.publish(self.gripper_command)
        rospy.sleep(0.1)
    ...
    def open_gripper(self):
        ...
        # self.client.send_goal(g)
        # self.client.wait_for_result()
        # self.gripper_pub.publish(self.genCommand('a', command))
        self.gripper_command = self.genCommand('o', self.gripper_command)
        self.gripper_pub.publish(self.gripper_command)
        rospy.sleep(0.1)
    ...
    def test_move(self):
        for _ in range(3):
        # while True:
            self.go_to_init_joints()
            self.close_gripper()
            self.go_to_joints(self.up_joints)
            self.open_gripper()
